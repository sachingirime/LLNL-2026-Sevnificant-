...
def skeletonize(input_filepath: str, output_filepath: str) -> str:
    """
    Create a skeleton from a 3D segmentation mask file and save it to disk.

    Args:
        input_filepath: Path to the .npy file containing the 3D binary mask.
        output_filepath: Path where the skeleton .npy will be saved.
    ...
    try:
        ...
        return f"Error: could not create output directory: {err}"

    try:
        # Delegate to the shared implementation in `src/skeletonization.py`.
        skeleton = skeletonize_mask(input_filepath, output_filepath)
    except Exception as err:
        return f"Error: skeletonization failed: {err}"

    # If the implementation returned an array, ensure it's persisted to disk.
    if skeleton is not None:
        try:
            # `skeleton` may already be a numpy array. Save it to the requested
            # output file to ensure the tool produces the expected artifact.
            if not os.path.exists(output_filepath):
                np.save(output_filepath, skeleton)
        except Exception as err:
            return f"Error: failed saving skeleton output: {err}"

    # If the implementation returns None, try loading the saved file.
    if skeleton is None:
        if os.path.exists(output_filepath):
            try:
                skeleton = np.load(output_filepath)
            except Exception as err:
                return f"Error: skeleton saved but could not be loaded: {err}"
        else:
            return "Error: skeletonization did not produce output"

    nonzero = int(np.count_nonzero(skeleton))
    return (
        f"Saved skeleton to {output_filepath} (shape={skeleton.shape}, "
        f"nonzero_voxels={nonzero})"
    )
